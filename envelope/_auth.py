...
class AuthResult:
    ...
    @staticmethod
    def parse_auth_headers(headers: EmailMessage) -> "AuthResult":
        ...
        # Extract from text
        results = AuthResult._extract_results_from_text(combined_auth_results)
        ...
        results["spf_received"] = spf_received
        ...
        elif any(v == "fail" for v in [spf, results.get("dkim"), results.get("dmarc"), spf_received]):
            verdict = "fail"
            if not failure_reason:
                failure_reason = "At least one mechanism failed"
        elif any(v == "softfail" for v in [spf, spf_received]):
            verdict = "softfail"
            failure_reason = failure_reason or "Softfail detected"
        elif all(v == "pass" for v in [spf, results.get("dkim"), results.get("dmarc")] if v):
            verdict = "pass"
        else:
            verdict = "partial/passive"
